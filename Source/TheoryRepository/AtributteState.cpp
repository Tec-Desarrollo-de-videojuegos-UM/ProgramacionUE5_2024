...
void UAtributteState::Checkhealth()
{
	if (health >= 0)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5, FColor::Blue, TEXT("El personaje sigue vivo"));
	}
	else
	{
...
bool UAtributteState::UseStamina(float StaminaToUse)
{
	if (stamina <=0)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5, FColor::Blue, TEXT("Stamina Insuficiente"));
		return false;
	}
	else
	{
		stamina = stamina - StaminaToUse;
		GEngine->AddOnScreenDebugMessage(-1, 5, FColor::Red, TEXT("Stamina Disponible"));
		return false;
	}
